...
import { Route } from './download'

describe('Route /download — redirect to locale download', () => {
  it('throws a redirect Response', () => {
    expect(() => Route.options.beforeLoad({} as any)).toThrow()
  })

  it('redirects to /$locale/download with DEFAULT_LOCALE param', () => {
    let thrown: unknown
    try {
    ...
    expect(r.options.to).toBe('/$locale/download')
